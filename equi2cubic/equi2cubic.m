function [out] = equi2cubic(im, yawChange, output_size, vfov )
...
% In degrees
views = [0+yawChange 0 0; % Front
         90+yawChange 0 0; % Right
         180+yawChange 0 0; % Back
         -90+yawChange 0 0; % Left
         0+yawChange 90 0; % Top
         0+yawChange -90 0]; % Bottom
     
for indexFirstColumn = 1:6
    if(views(indexFirstColumn,1)>180)
        views(indexFirstColumn,1) = views(indexFirstColumn,1)-360;
    elseif(views(indexFirstColumn,1)<-180)
        views(indexFirstColumn,1) = views(indexFirstColumn,1)+360;
    end
end
